...
public class FlexBase64 {
    ...
    * 
    * <p>
    * The Encoder instance is not thread-safe, and must not be shared between
    * threads without establishing a happens-before relationship.
    * </p>
    * 
    * @param wrap
    *           whether or not to wrap at 76 characters with CRLF
    ...
    * 
    * <p>
    * The Decoder instance is not thread-safe, and must not be shared between
    * threads without establishing a happens-before relationship.
    * </p>
    * 
    ...
    * 
    * <p>
    * This method is only useful for applications which require a String and
    * have all data to be encoded up-front. Note that byte arrays or buffers are
    * almost always a better storage choice. They consume half the memory and
    * can be reused (modified). In other words, it is almost always better to
    * use {@link #encodeBytes}, {@link #createEncoder}, or
    * {@link #createEncoderOutputStream} instead. instead.
    * 
    * @param source
    *           the byte array to encode from
    * @param wrap
    *           whether or not to wrap the output at 76 chars with CRLFs
    ...
    * can be reused (modified). In other words, it is almost always better to
    * use {@link #encodeBytes}, {@link #createEncoder}, or
    * {@link #createEncoderOutputStream} instead.
    * </p>
    * 
    * <pre>
    * <code>
    ...
    *           the byte array to encode from
    * @param pos
    *           the position to start encoding from
    * @param limit
    *           the position to halt encoding at (exclusive)
    * @param wrap
    *           whether or not to wrap the output at 76 chars with CRLFs
    * @return a new String representing the Base64 output
    */
    ...
    * 
    * @param source
    *           the byte buffer to encode from
    * @param wrap
    *           whether or not to wrap the output at 76 chars with CRLFs
    * @return a new String representing the Base64 output
    */
    ...
    *           the byte array to encode from
    * @param pos
    *           the position to start encoding at
    * @param limit
    *           the position to halt encoding at (exclusive)
    * @param wrap
    *           whether or not to wrap at 76 characters with CRLFs
   ...
   /**
    * Decodes a Base64 encoded string into a new byte buffer. The returned byte
    * buffer is a heap buffer, and it is therefor possible to retrieve the
    * backing array using {@link java.nio.ByteBuffer#array()},
    * {@link java.nio.ByteBuffer#arrayOffset()} and
    * {@link java.nio.ByteBuffer#limit()}. The latter is very important since
    * the decoded array may be larger than the decoded data. This is due to
    * length estimation which avoids an unnecessary array copy.
    * 
    * @param source
   ...
   /**
    * Decodes a Base64 encoded byte buffer into a new byte buffer. The returned
    * byte buffer is a heap buffer, and it is therefor possible to retrieve the
    * backing array using {@link java.nio.ByteBuffer#array()},
    * {@link java.nio.ByteBuffer#arrayOffset()} and
    * {@link java.nio.ByteBuffer#limit()}. The latter is very important since
    * the decoded array may be larger than the decoded data. This is due to
   ...
   /**
    * Decodes a Base64 encoded byte array into a new byte buffer. The returned
    * byte buffer is a heap buffer, and it is therefor possible to retrieve the
    * backing array using {@link java.nio.ByteBuffer#array()},
    * {@link java.nio.ByteBuffer#arrayOffset()} and
    * {@link java.nio.ByteBuffer#limit()}. The latter is very important since
    ...
    *           the Base64 content to decode
    * @param off
    *           position to start decoding from in source
    * @param limit
    *           position to stop decoding in source (exclusive)
    * @return a byte buffer containing the decoded output
    * @throws IOException
    *            if the encoding is invalid or corrupted
    */
   ...
   /**
    * Creates an InputStream wrapper which encodes a source into base64 as it is
    * read, until the source hits EOF. Upon hitting EOF, a standard base64
    * termination sequence will be readable. Clients can simply treat this input
    * stream as if they were reading from a base64 encoded file. This stream
    * attempts to read and encode in buffer size chunks from the source, in
    * order to improve overall performance. Thus, BufferInputStream is not
    * necessary and will lead to double buffering.
    * 
    * <p>
    * This stream is not thread-safe, and should not be shared between threads,
    * without establishing a happens-before relationship.
    * </p>
    * 
    * @param source
    *           an input source to read from
    * @param bufferSize
    *           the chunk size to buffer from the source
    * @param wrap
    *           whether or not the stream should wrap base64 output at 76
    *           characters
    ...
    * termination sequence will be readable. Clients can simply treat this input
    * stream as if they were reading from a base64 encoded file. This stream
    * attempts to read and encode in 8192 byte chunks. Thus, BufferedInputStream
   ...
   /**
    * Creates an InputStream wrapper which decodes a base64 input source into
    * the decoded content as it is read, until the source hits EOF. Upon hitting
    * EOF, a standard base64 termination sequence will be readable. Clients can
    * simply treat this input stream as if they were reading from a base64
    * encoded file. This stream attempts to read and encode in buffer size byte
    * chunks. Thus, BufferedInputStream is not necessary as a source and will
    * lead to double buffering.
    * 
    * <p>
    * Note that the end of a base64 stream can not reliably be detected, so if
    * multiple base64 streams exist on the wire, the source stream will need to
    * simulate an EOF when the boundary mechanism is detected.
    * </p>
    * 
    * <p>
    * This stream is not thread-safe, and should not be shared between threads,
    ...
    *           an input source to read from
    * @param bufferSize
    *           the chunk size to buffer before when reading from the target
    ...
    * EOF, a standard base64 termination sequence will be readable. Clients can
    * simply treat this input stream as if they were reading from a base64
    * encoded file. This stream attempts to read and encode in 8192 byte chunks.
    * Thus, BufferedInputStream is not necessary as a source and will lead to
    * double buffering.
    * 
    * <p>
    * Note that the end of a base64 stream can not reliably be detected, so if
   ...
   /**
    * Creates an OutputStream wrapper which base64 encodes and writes to the
    * passed OutputStream target. When this stream is closed base64 padding will
    * be added if needed. Alternatively if this represents an "inner stream",
    * the {@link FlexBase64.EncoderOutputStream#complete()} method can be called
    * to close out the inner stream without closing the wrapped target.
    * 
    * <p>
    * All bytes written will be queued to a buffer in the specified size. This
    * stream, therefore, does not require BufferedOutputStream, which would lead
    * to double buffering.
    * 
    * @param target
    *           an output target to write to
    * @param bufferSize
    *           the chunk size to buffer before writing to the target
    * @param wrap
    *           whether or not the stream should wrap base64 output at 76
    *           characters
    ...
    * 
    * <p>
    * All bytes written will be queued to an 8192 byte buffer. This stream,
    * therefore, does not require BufferedOutputStream, which would lead to
    * double buffering.
    * </p>
    * 
    * <p>
    ...
    * 
    * @param output
    *           the output stream to write encoded output to
    * @return an encoded output stream instance.
    */
   ...
   /**
    * Creates an OutputStream wrapper which decodes base64 content before
    * writing to the passed OutputStream target.
    * 
    * <p>
    * All bytes written will be queued to a buffer using the specified buffer
    * size. This stream, therefore, does not require BufferedOutputStream, which
    * would lead to double buffering.
    * </p>
    * 
    * <p>
    * This stream is not thread-safe, and should not be shared between threads,
    ...
    * @param bufferSize
    *           the buffer size to buffer writes to
   ...
   public static final class Encoder {
      ...
      /**
       * Encodes bytes read from source and writes them in base64 format to
       * target. If the source limit is hit, this method will return and save
       * the current state, such that future calls can resume the encoding
       * process. In addition, if the target does not have the capacity to fit
       * an entire quad of bytes, this method will also return and save state
       * for subsequent calls to this method. Once all bytes have been encoded
       * to the target, {@link #complete(java.nio.ByteBuffer)} should be called
       * to add the necessary padding characters.
       * 
       * @param source
      ...
      public void encode(ByteBuffer source, ByteBuffer target) {
         ...
         while (remaining > 0) {
            // Unrolled state machine for performance (resumes and executes all
            // states in one iteration)
            int require = 4 - state;
       ...
       * an entire quad of bytes, this method will also return and save state
       * for subsequent calls to this method. Once all bytes have been encoded
       * to the target, {@link #complete(byte[], int)} should be called to add
       * the necessary padding characters. In order to determine the last read
       * position, the {@link #getLastInputPosition()} can be used.
       * 
       * <p>
       * Note that the limit values are not lengths, they are positions similar
       * to {@link java.nio.ByteBuffer#limit()}. To calculate a length simply
       * subtract position from limit.
       * </p>
       * 
       * <pre>
       * <code>
       ...
       * @param limit
       *           the position in the byte array that is after the end of the
       *           source data
       * @param target
       *           the byte array to write base64 bytes to
       * @param opos
       *           the position to start writing to the target array at
       * @param olimit
       *           the position in the target byte array that makes the end of
       *           the writable area (exclusive)
       * @return the position in the target array immediately following the last
       *         byte written
       */
      ...
      /**
       * Gets the last position where encoding left off in the last byte array
       * that was used. If the target for encoded content does not have the
       * necessary capacity, this method should be used to determine where to
       * start from on subsequent reads.
       * 
       ...
       * This is essential to complying with the Base64 format. This method will
       * write at most 4 or 2 bytes starting at pos,depending on whether or not
       * wrapping is enabled.
       * 
       * <pre>
       * <code>
       ...
       * Completes an encoding session by writing out the necessary padding.
       * This is essential to complying with the Base64 format. This method will
       * write at most 4 or 2 bytes, depending on whether or not wrapping is
       * enabled.
       * 
       * @param target
       *           the byte buffer to write to
   ...
   public static final class Decoder {
      ...
      /**
       * Decodes one Base64 byte buffer into another. This method will return
       * and save state if the target does not have the required capacity.
       * Subsequent calls with a new target will resume reading where it last
       * left off (the source buffer's position). Similarly not all of the
       * source data need be available, this method can be repetitively called
       * as data is made available.
       * 
       * <p>
       * The decoder will skip white space, but will error if it detects
       * corruption.
       * </p>
       * 
       * @param source
       *           the byte buffer to read encoded data from
       * @param target
       *           the byte buffer to write decoded data to
       * @throws IOException
       *            if the encoded data is corrupted
       */
      ...
      private int decode(Object source, int sourcePos, int sourceLimit, byte[] target, int targetPos, int targetLimit)
            throws IOException {
         if (target == null)
            throw new IllegalStateException();
      ...
      /**
       * Gets the last position where decoding left off in the last byte array
       * that was used for reading. If the target for decoded content does not
       * have the necessary capacity, this method should be used to determine
       * where to start from on subsequent decode calls.
       * 
       * @return the last known read position
       */
      public int getLastInputPosition() {
      ...
      /**
       * Decodes one Base64 byte array into another byte array. If the source
       * limit is hit, this method will return and save the current state, such
       * that future calls can resume the decoding process. Likewise, if the
       * target does not have the capacity, this method will also return and
       * save state for subsequent calls to this method.
       * 
       * <p>
       * When multiple calls are made, {@link #getLastInputPosition()} should be
       * used to determine what value should be set for sourcePos. Likewise, the
       * returned target position should be used as the targetPos in a
       * subsequent call.
       * </p>
       * 
       * <p>
       ...
       * @param sourcePos
       *           the position in the source array to start decoding from
       * @param sourceLimit
       *           the position in the source array to halt decoding when hit
       *           (exclusive)
       * @param target
       *           the byte buffer to write decoded data to
       * @param targetPos
       *           the position in the target byte array to begin writing at
       * @param targetLimit
       *           the position in the target byte array to halt writing
       *           (exclusive)
       * @throws IOException
       *            if the encoded data is corrupted
       * @return the position in the target array immediately following the last
       ...
       * 
       */
      public int decode(String source, int sourcePos, int sourceLimit, byte[] target, int targetPos, int targetLimit)
            throws IOException {
         return decode((Object) source, sourcePos, sourceLimit, target, targetPos, targetLimit);
      }

      /**
       * Decodes a Base64 encoded string into the passed byte array. This method
       * will return and save state if the target does not have the required
       * capacity. Subsequent calls with a new target will resume reading where
       * it last left off (the source buffer's position). Similarly not all of
       * the source data need be available, this method can be repetitively
       * called as data is made available.
       * 
       * <p>
       * Since this method variant assumes a position of 0 and a limit of the
       * item length, repeated calls will need fresh source and target values.
       * {@link #decode(String, int, int, byte[], int, int)} would be a better
       * fit if you need reuse
       * </p>
       * 
       * <p>
       * The decoder will skip white space, but will error if it detects
       ...
       * 
       * @param source
       *           a base64 encoded string to decode from
       * @param target
       *           a byte array to write to
       * @throws java.io.IOException
       *            if the base64 content is malformed
       ...
       * 
       * @param source
       *           the byte array to read encoded data from
       * @param sourcePos
       *           the position in the source array to start decoding from
       * @param sourceLimit
       *           the position in the source array to halt decoding when hit
       ...
       *         byte written
       */
      public int decode(byte[] source, int sourcePos, int sourceLimit, byte[] target, int targetPos, int targetLimit)
            throws IOException {
         return decode((Object) source, sourcePos, sourceLimit, target, targetPos, targetLimit);
      }
   ...
   /**
    * An input stream which decodes bytes as they are read from a stream with
    * Base64 encoded data.
    */
   ...
   public static class EncoderInputStream extends InputStream {
      ...
      public int read(byte[] b, int off, int len) throws IOException {
         ...
         for (;;) {
            ...
            if (pos >= limit) {
               if (len > source.length) {
                  // If requested length exceeds buffer, allocate a new
                  // temporary buffer that will be
                  // one block less than an exact encoded output. This is to
                  // handle partial quad carryover
   ...
   /**
    * An output stream which base64 encodes all passed data and writes it to the
    * wrapped target output stream.
    * 
    * <p>
    * Closing this stream will result in the correct padding sequence being
    * written. However, as required by the OutputStream contract, the wrapped
    * stream will also be closed. If this is not desired, the
    * {@link #complete()} method should be used.
    * </p>
    */
   ...
   /**
    * An output stream which decodes base64 data written to it, and writes the
    * decoded output to the wrapped inner stream.
    */
